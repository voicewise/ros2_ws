...
class PacketFunction(enum.IntEnum):
    # Funzioni di controllo disponibili tramite porta serialecontrollo disponibili tramite porta seriale
    PACKET_FUNC_SYS = 0
...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=1000000, timeout=5):
        ...
        # La coda viene utilizzata per archiviare i dati
    ...
    def get_button(self):
        # Ottieni lo stato di key1 e key2 e restituisci l'ID della chiave (1 significa pulsante 1, 2 significa pulsante 2) e lo stato (0 significa premuto, 1 significa rilasciato)
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # Ottieni i dati IMU e restituisci ax, ay, az, gx, gy, gz
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
