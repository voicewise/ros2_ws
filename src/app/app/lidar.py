...
class LidarController(Node):
    ...
    def __init__(self, name):
        super().__init__(name)
        self.running_mode = 0               # 1: Modalità di evitamento degli ostacoli radar 2: Modalità guardia radar (1: Radar obstacle avoidance mode, 2: Radar guard mode)
        self.threshold = 0.9                # meters  soglia di distanza (distance threshold)
        self.scan_angle = math.radians(90)  # radians  angolo di scansione in avanti (the forward scanning angle)
        self.speed = 0.12                   # Metro unitario, velocità in modalità evitamento ostacoli (speed in meters per second for obstacle avoidance mode)
        self.timestamp = 0
        self.lock = RLock()
        self.lidar_sub = None
    ...
    def lidar_callback(self, lidar_data: LaserScan):
        ...
        with self.lock:
            ...
            # Gestione aggiramento ostacoli (obstacle avoidance)
