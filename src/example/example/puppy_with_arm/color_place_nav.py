...
Bend = {'roll': math.radians(0), 'pitch': math.radians(-17.0), 'yaw': 0.0, 'height': -10.0, 'x_shift': 0.5, 'stance_x': 0.0, 'stance_y': 0.0}
Stand = {'roll': math.radians(0), 'pitch': math.radians(0), 'yaw': 0.0, 'height': -10.0, 'x_shift': -1.0, 'stance_x': 0.0, 'stance_y': 0.0}
GaitConfig = {'overlap_time': 0.15, 'swing_time': 0.15, 'clearance_time': 0.0, 'z_clearance': 3.0}

PuppyPose = Bend.copy()
PuppyMove = {'x': 0.0, 'y': 0.0, 'yaw_rate': 0.0}
...
class HandControlWithArmNode(Node):
    def __init__(self):
        ...
        self.goal_status_subscriber = self.create_subscription(String, '/move_base/status', self.goal_status_callback, 1)
    ...
    def init_pose(self):
        PuppyPose['stance_x'] = float(PuppyPose.get('stance_x', 0.0))
        PuppyPose['stance_y'] = float(PuppyPose.get('stance_y', 0.0))
        PuppyPose['x_shift'] = float(PuppyPose.get('x_shift', 0.0))
        PuppyPose['height'] = float(PuppyPose.get('height', 0.0))
        PuppyPose['roll'] = float(PuppyPose.get('roll', 0.0))
        PuppyPose['pitch'] = float(PuppyPose.get('pitch', 0.0))
        PuppyPose['yaw'] = float(PuppyPose.get('yaw', 0.0))
    ...
    def goal_status_callback(self, msg):
        status = msg.data
        if status == 'success' and not self.nav_status:
