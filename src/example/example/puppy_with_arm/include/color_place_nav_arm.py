...
class HandControlWithArmNode(Node):
    def __init__(self):
        ...
        # 获取参数值并将其转换为字典
        color_range_str = self.get_parameter('/lab_config_manager/color_range_list').value

        try:
            # 将参数字符串解析为字典
            self.color_range_list = json.loads(color_range_str)
        except json.JSONDecodeError:
            self.get_logger().error('Failed to parse color_range_list from parameter server, using empty dictionary')
    ...
    def init_pose(self):
        ...
        # 确保 run_time 是 int 类型
        run_time = int(500)  # 将 500 转换为 int 类型
        ...
        self.puppy_pose_pub.publish(Pose(
            ...
            yaw=PuppyPose['yaw'],
            run_time=run_time  # 传递 int 类型的 run_time
        ))
    ...
    def image_callback(self, ros_image):
        """
        图像订阅回调，将ROS2的Image消息转换为OpenCV图像
        """
        bridge = CvBridge()
