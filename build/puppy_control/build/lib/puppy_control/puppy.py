...
LookDown = {'roll': math.radians(0), 'pitch': math.radians(-15.0), 'yaw': 0.000, 'height': -10.0, 'x_shift': -0.5, 'stance_x': 0, 'stance_y': 0}
LookDown_10deg = {'roll': math.radians(0), 'pitch': math.radians(-10.0), 'yaw': 0.000, 'height': -9.0, 'x_shift': -0.1, 'stance_x': 0, 'stance_y': 0}
LookDown_20deg = {'roll': math.radians(0), 'pitch': math.radians(-20.0), 'yaw': 0.000, 'height': -9.0, 'x_shift': -0.1, 'stance_x': 0, 'stance_y': 0}
...
class PUPPY(Node):
    def __init__(self):
        ...
        # 初始化HiwonderPuppy
        self.puppy = HiwonderPuppy(setServoPulse=self.setServoPulse, servoParams=PWMServoParams(), dof='8')
    ...
    def setServoPulse(self, ids, pulse, use_time):

        # self.get_logger().info(str(ids) + ' '+ str(pulse))
        
        # if ids == 8:
            # self.get_logger().info('\n')
        psc.setPulse(ids, pulse, use_time)
    ...
    def pub(self):
        # 定义日志文件的目录和路径
        # log_dir = '/home/ubuntu/share/tmp/'
        # log_file_path = os.path.join(log_dir, 'joint_angles_log.txt')

        # 检查目录是否存在，不存在则创建
        # if not os.path.exists(log_dir):
            # os.makedirs(log_dir)
            # print(f"目录 {log_dir} 不存在，已创建。")

        # 添加标志位，确保日志路径只打印一次
        # if not hasattr(self, 'log_printed'):
        ...
        if self.joint_state_pub_topic.lower() == 'true' or self.joint_state_controller_pub_topic.lower() == 'true':
            ...
            # 转换角度为度数并打印
            joint_angles_in_degrees = joint_angles * 57.3
            # self.get_logger().info('\n' + str(joint_angles_in_degrees))
            # 获取当前时间，并计算相对时间
            current_time = time.time()
            # self.get_logger().info(str(time.time() - self.start_time))
            self.start_time = time.time()
            ...
            # try:
                ...
                # if not self.log_printed:
                    # print(f"Joint angles logged to {log_file_path}")
                    # self.log_printed = True  # 设置标志位，确保日志路径只打印一次
